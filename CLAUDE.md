...
- `src/mcp/server.ts` — stdio MCP server (`latoile-mcp` bin, `yarn mcp`) exposing `get_context(jiraKey, maxDepth?, maxNodes?, refresh?)`; the tool handler (`getContextTool`) is exported separately for tests. Progress logs go to stderr (stdout is the JSON-RPC channel — never console.log there).
