...
- `src/mcp/server.ts` — stdio MCP server (`latoile-mcp` bin, `yarn mcp`) with four tools: `get_context` (full traversal), `get_context_from_mr` (GitLab MR URL entry point — `src/collector/mr-entry.ts` parses the URL and extracts the Jira key from source branch → title → description), `search_issues` (JQL text search, shared logic in `src/collector/search.ts`), `get_issue` (single fetch, no traversal). All tools return `structuredContent` with an output schema; pipeline logs are forwarded as MCP logging notifications and, when the client sends a `progressToken`, as progress notifications. Handlers (`getContextTool`, `searchIssuesTool`, `getIssueTool`) are exported separately for tests (`test/mcp.test.ts`). Progress logs go to stderr (stdout is the JSON-RPC channel — never console.log there).
